<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:77:"F:\phpstudy\WWW\sugoo\admin\public/../application/index\view\cupon\cupon.html";i:1515205370;}*/ ?>
...
$(document).on("click","#eidt",function(){
  ...
  // 申请人
  var ca_make = $("#mark_man").text();
  // 用途
  var ca_use = $("#cu_use").text();
  // 审核人
  var ca_name = $("#audit_man").text();
  // 红包个数
  var cu_num = $("#cu_num").attr("value");
  // 审核状态
  var ca_status = $("#audit_status").attr("value");
  // 红包面值
  var ca_money = $("#cu_money").text();
  alert(ca_money);
  // alert(ca_status);
  // 数据组装
  // var data = {
    ...
    "type":"get",
     success:function(msg){
      if(msg.code==1){
        ...
        <td align='center' id='mark_man'>"+msg.page_info[i].mark_man+"</td>\
        <td align='center' id='cu_use'>"+msg.page_info[i].cu_use+"</td>\
        <td align='center' id='audit_man'>"+msg.page_info[i].audit_man+"</td>\
        <td align='center' id='audit_status' value="+msg.page_info[i].audit_status+">"+msg.page_info[i].audit_status+"</td>\
        <td align='center'>"+msg.page_info[i].audit_time+"</td>\
        <td align='center' id='cu_money' value="+msg.page_info[i].cu_money+">"+msg.page_info[i].cu_money+"元</td>\
        <td align='center'>"+msg.page_info[i].su_limit+"天</td>\
        <td align='center' id='cu_num' value="+msg.page_info[i].cu_num+">"+msg.page_info[i].cu_num+"个</td>\
